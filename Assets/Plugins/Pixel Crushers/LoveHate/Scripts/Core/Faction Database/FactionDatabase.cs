...
namespace PixelCrushers.LoveHate
{
    ...
    public class FactionDatabase : ScriptableObject
    {
        ...
        public void SetPersonalityTrait(int factionID, int traitID, float value)
        {
            ...
            {
                var min = personalityTraitDefinitions[traitID].minValue;
                var max = personalityTraitDefinitions[traitID].maxValue;
                var clampedValue = Mathf.Clamp(value, min, max);
                faction.traits[traitID] = clampedValue;
                personalityTraitChanged(factionID, traitID, clampedValue);
            }
        }
        ...
        public void SetPersonalRelationshipTrait(int judgeFactionID, int subjectFactionID, int traitID, float value)
        {
            ...
            if (0 <= traitID && traitID < relationshipTraitDefinitions.Length)
            {
                var min = relationshipTraitDefinitions[traitID].minValue;
                var max = relationshipTraitDefinitions[traitID].maxValue;
                clampedValue = Mathf.Clamp(value, min, max);
            }
            if (Mathf.Approximately(0, faction.percentJudgeParents))
            {
                faction.SetPersonalRelationshipTrait(subjectFactionID, traitID, clampedValue, relationshipTraitDefinitions.Length);
            }
            else
            {
                var previousValue = GetRelationshipTrait(judgeFactionID, subjectFactionID, traitID);
                var change = clampedValue - previousValue;
